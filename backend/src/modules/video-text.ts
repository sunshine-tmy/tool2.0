...
type RemoteVideoInsightTranscript = {
  analysis?: VideoTextAnalysis;
  warning?: string;
};

export class VideoInsightUploadError extends Error {
  constructor(
...
export async function transcribeUploadedVideoForInsight(options: {
  config: AppConfig;
  ...
  const { config, stream, mimeType, maxBytes } = options;
  if (!config.videoTextTranscribeCommand) {
    stream.resume();
    throw new VideoInsightUploadError(
      "VIDEO_TRANSCRIBER_NOT_CONFIGURED",
      409,
      "未配置本地视频转写命令，无法分析上传视频"
    );
  }
  if (!mimeType.startsWith("video/")) {
    stream.resume();
    throw new VideoInsightUploadError("VIDEO_INSIGHT_VIDEO_REQUIRED", 415, "请上传受支持的视频文件");
  }

  const safeName = sanitizeDisplayFileName(options.fileName || "uploaded-video.mp4");
  const extension =
    path
      .extname(safeName)
      .toLowerCase()
      .replace(/[^.a-z0-9]/g, "")
      .slice(0, 12) || ".mp4";
  const taskId = `insight-upload-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  const videoPath = path.join(config.videoTextUploadsDir, `${taskId}${extension}`);
  let fileSize = 0;
  const limiter = new Transform({
    transform(chunk: Buffer, _encoding, callback) {
      fileSize += chunk.length;
      if (fileSize > maxBytes) {
        callback(new VideoInsightUploadError("VIDEO_INSIGHT_UPLOAD_TOO_LARGE", 413, "上传视频超过允许大小"));
        return;
      }
      callback(null, chunk);
    }
  ...
  try {
    ...
    await pipeline(stream, limiter, fs.createWriteStream(videoPath));
    if (stream.truncated) {
      throw new VideoInsightUploadError("VIDEO_INSIGHT_UPLOAD_TOO_LARGE", 413, "上传视频超过允许大小");
    }
    const transcribed = await transcribeVideo(videoPath, taskId, config);
    if (!transcribed.transcript.trim()) {
      throw new VideoInsightUploadError("VIDEO_INSIGHT_TRANSCRIPT_EMPTY", 422, "视频中未识别到可用语音文本");
    }
    return {
      analysis: analyzeVideoText({
        title: safeName,
    ...
    if (error instanceof VideoInsightUploadError) throw error;
    throw new VideoInsightUploadError(
      "VIDEO_INSIGHT_TRANSCRIPTION_FAILED",
      422,
      `上传视频转写失败：${error instanceof Error ? error.message : "未知错误"}`
    );
  } finally {
    await Promise.all([
...
/**
 * Reuses the same guarded remote download and local transcription pipeline used by
 * the video-text tool, while ensuring the fetched media is removed afterwards.
 */
export async function transcribeRemoteVideoForInsight(options: {
  config: AppConfig;
  remoteFetch: RemoteFetch;
  sourceUrl: string;
  title: string;
}): Promise<RemoteVideoInsightTranscript> {
  const { config, remoteFetch, sourceUrl, title } = options;
  if (!config.videoTextTranscribeCommand) {
    return { warning: "未配置本地视频转写命令，已基于标题和文案执行规则拆解。" };
  }

  const taskId = `insight-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  const extension = path
    .extname(fileNameFromUrl(sourceUrl))
    .toLowerCase()
    .replace(/[^.a-z0-9]/g, "")
    .slice(0, 12);
  const videoPath = path.join(config.videoTextUploadsDir, `${taskId}${extension || ".mp4"}`);

  try {
    const response = await fetchRemoteVideo(sourceUrl, remoteFetch, config);
    if (!response.ok || !response.body) {
      return { warning: `媒体下载失败（HTTP ${response.status}），已基于标题和文案执行规则拆解。` };
    }
    assertRemoteResponseSize(response, config.remoteMediaMaxBytes);
    await fsp.mkdir(config.videoTextUploadsDir, { recursive: true });
    await pipeline(limitedResponseStream(response, config.remoteMediaMaxBytes), fs.createWriteStream(videoPath));

    const transcribed = await transcribeVideo(videoPath, taskId, config);
    if (!transcribed.transcript.trim()) {
      return { warning: "未识别到可用视频语音，已基于标题和文案执行规则拆解。" };
    }
    return {
      analysis: analyzeVideoText({
        title: sanitizeDisplayFileName(title || "竞品视频"),
        transcript: transcribed.transcript,
        recognitionQuality: transcribed.recognitionQuality
      })
    };
    ...
    return {
      warning: `本地转写未完成：${error instanceof Error ? error.message : "未知错误"}`
    };
  } finally {
    await Promise.all([
      fsp.rm(videoPath, { force: true }),
