...
- 分支：`main`
- 合并基线提交：`24a822e`（PR #18 已合并）；F06 启用提交：`529220c`
- `pnpm check`：通过
...
F06 的检查器位于 `scripts/check-diff-coverage.mjs`，并由 `test:diff-coverage` 覆盖其解析逻辑。合并后的 `main` 已启用 `diff-coverage` Job：每个 Pull Request 自动读取 PR base 与三份 LCOV，并以 90% 变更可执行行覆盖率作为阻断门槛。
...
| F06 | DONE | 启用变更行覆盖率 90%         | 合并后的 main 基线 | 从 PR base 与 LCOV 计算新增/修改可执行行，低于 90% 阻止 CI                       |
...
| H02 | DONE | 最终企业级验收      | 全部任务      | 干净 checkout 完成全部门禁、迁移、基准、E2E 和双平台分发，媒体不丢失 |
...
- 变更行覆盖率 90% 已在合并后的 `main` 基线启用，并对后续 Pull Request 生效。
...
| F06     | DONE | 2026-09-15 | `529220c` | `pnpm test:diff-coverage`（3 项）；`pnpm coverage:diff --base origin/main`                                                                                                        | `pnpm check` 通过 | 合并后的 main 新基线启用 PR `diff-coverage`；本地相对当前基线无新增业务可执行行，0/0 = 100%，90% 门槛已生效                                                                                                     | CI 每个 Pull Request 自动计算 PR base 与三份 LCOV；低于 90% 阻断，未修改现有 storage                                                         |
...
| H02     | DONE | 2026-09-15 | `[H02]`   | `pnpm audit --prod --audit-level high`；`pnpm db:migrate --dry-run`；`pnpm db:verify`；`pnpm db:benchmark`；`pnpm test:e2e`；Windows ZIP/macOS TAR.GZ 解压安装构建 smoke          | `pnpm check` 通过 | 合并后 main 本机最终验收通过：依赖无高危、SQLite integrity ok/schema v5、列表 P95 14.83 ms、详情 P95 0.04 ms、E2E 4 项、双平台归档健康/上传/下载/清理/退出通过；CI 聚合门禁已包含 F06                           | 未修改现有 storage 媒体、音色、归档或迁移备份；未在本地创建远程 Release                                                                      |
