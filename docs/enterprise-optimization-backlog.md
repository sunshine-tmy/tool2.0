...
F06 的检查器已预置在 `scripts/check-diff-coverage.mjs`，并由 `test:diff-coverage` 覆盖其解析逻辑。CI 的 `diff-coverage` Job 默认关闭，只有在仓库变量 `ENABLE_DIFF_COVERAGE=true` 且 Pull Request 事件中才会读取 PR base 与三份 LCOV 并以 90% 阈值失败。当前分支尚未形成合并后的新基线，因此变量不得开启，F06 继续保持 `BLOCKED_BY_BASELINE`。
...
| H02 | IN_PROGRESS | 最终企业级验收      | 全部任务      | 干净 checkout 完成全部门禁、迁移、基准、E2E 和双平台分发，媒体不丢失 |
...
| F06     | BLOCKED_BY_BASELINE | 2026-09-15 | `[F06]`   | `pnpm test:diff-coverage`（3 项）；`pnpm coverage:diff --base 76c65b6 --report-only`                                                                                                     | `pnpm check` 通过 | 当前基线只读预检：1033/1254 = 82.38%，低于 90%；CI `diff-coverage` 默认关闭，待合并形成新基线后由 `ENABLE_DIFF_COVERAGE=true` 开启                                                                              | 检查器和 LCOV artifact 门禁已预置，但 F06 尚未验收完成，保持 `BLOCKED_BY_BASELINE`                                                           |
...
| H02     | IN_PROGRESS         | 2026-09-15 | `[H02]`   | `pnpm audit --prod --audit-level high`；`pnpm db:migrate --dry-run`；`pnpm db:migrate`（隔离临时 storage）；`pnpm db:verify`；`pnpm db:benchmark`；`pnpm test:e2e`；真实双平台归档 smoke | `pnpm check` 通过 | 本机预验收：依赖无高危、迁移/完整性/基准通过（列表 P95 12.73 ms、详情 0.03 ms）、E2E 4 项、ZIP/TAR.GZ 解压安装构建传输退出通过；CI 已加入聚合 enterprise-acceptance 门禁                                        | H02 暂不标记 DONE：F06 仍为 `BLOCKED_BY_BASELINE`，须待本分支合并形成新基线后启用变更行覆盖率 90%；未修改现有 storage 媒体或远程 Release     |
