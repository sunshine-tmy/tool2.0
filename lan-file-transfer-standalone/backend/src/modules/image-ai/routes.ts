...
import { createImageAiInstaller } from "./installer";
...
export async function registerImageAiRoutes(app: FastifyInstance, config: AppConfig) {
  ...
  const installer = createImageAiInstaller(config);
  await manager.initialize();
  app.addHook("onClose", async () => manager.close());

  app.get("/api/tools/image-ai/health", async () => {
    const installation = await installer.status();
    try {
      return ok({ ...(await worker.health()), installation });
    } catch (error) {
      return ok({
        available: false,
  ...
  app.post("/api/tools/image-ai/install", async (_request, reply) => {
    try {
      const installation = await installer.install();
      return reply.code(installation.status === "installing" ? 202 : 200).send(ok(installation));
    } catch (error) {
      const message = error instanceof Error ? error.message : "无法启动 AI 环境安装";
      return reply.code(400).send(fail("IMAGE_AI_INSTALL_FAILED", message));
    }
  });

  app.post("/api/tools/image-ai/worker/start", async (_request, reply) => {
    try {
      await installer.startWorker();
      return reply.code(202).send(ok(await installer.status(), "AI 推理服务正在启动"));
    } catch (error) {
      const message = error instanceof Error ? error.message : "无法启动 AI 推理服务";
      return reply.code(400).send(fail("IMAGE_AI_WORKER_START_FAILED", message));
    }
  });
