...
export type WorkerImageAiHealth = Omit<ImageAiHealth, "installation">;
...
export function createImageAiWorkerClient(config: AppConfig) {
  return {
    async health(): Promise<WorkerImageAiHealth> {
      ...
      return requestWorker<WorkerImageAiHealth>(config, "/health", { method: "GET" }, 15000);
    },
